...
def decompose(eq):
    ...
    if len(v) != 1:
        return eq
    v = v[0]
    if not all(diffx(diffx(x.children[0], v)) == tree_form("d_0") for x in lst):
        return eq
    s = []
    final = []
    lst2 = Counter(lst)
    ...
    lst3 = solve(expand2(summation([solve(allocate()*lst4/x) for x in s])))
    used2 = {}
    if lst3.name == "f_add":
    ...
    for key in used2.keys():
        if key == 0:
            used3.append(TreeNode("f_eq", [solve(used2[key] - tree_form("d_1")), tree_form("d_0")]))
        else:
...
def integratex(equation, var="v_0", no_init=False):
    ...
    def varproc(term, var):
        ...
        return None
    if not no_init:
        equation = solve(expand2(replace_eq3(equation)))
    ...
    eq = equation
    if not no_init:
        eq2 = solve(dowhile(eq, expand2))
        if eq.name in ["f_mul", "f_pow"] and eq2.name == "f_add":
            tmp4 = integratex(eq2, var, True)
            if tmp4 is not None:
                return tmp4
    if eq.name == "f_add":
        success = True
    ...
    if tmp is not None:
        ...
        a = tree_form(tmp["p_0"])
        b = tree_form(tmp["p_1"])
        if x.name == var and solve(a+b)==tree_form("d_0"):
            if int(a.name[2:]) <0:
                a, b = b, a
            return ((x-a)/(x+a)).fx("abs").fx("log")/(tree_form("d_2")*a)
    tmp = structure(eq, tree_form('f_pow\n f_add\n  f_pow\n   u_0\n   d_2\n  p_0\n d_-1'))
    if tmp is not None:
        x = tree_form(tmp["u_0"])
